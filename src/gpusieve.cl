...
const uint block_size_in_bytes = 8192;		// Size of shared memory array in bytes
const uint block_size = block_size_in_bytes * 8;	// Number of bits generated by each block
const uint threadsPerBlock = 256;			// Threads per block
#ifdef MORE_CLASSES
const uint primesNotSieved = 5;			// Primes 2, 3, 5, 7, 11 are not sieved
//const uint primesHandledWithSpecialCode = 13;	// Count of primes handled with inline code (not using primes array)
							// Primes 13 through 61 are handled specially
//const uint primesHandledWithSpecialCode = 26;	// Count of primes handled with inline code (not using primes array)
							// Primes 13 through 127 are handled specially
const uint primesHandledWithSpecialCode = 49;		// Count of primes handled with inline code (not using primes array)
							// Primes 13 through 251 are handled specially
//const uint primesHandledWithSpecialCode = 92;	// Count of primes handled with inline code (not using primes array)
							// Primes 13 through 509 are handled specially
#else
const uint primesNotSieved = 4;			// Primes 2, 3, 5, 7 are not sieved
//const uint primesHandledWithSpecialCode = 14;	// Count of primes handled with inline code (not using primes array)
							// Primes 11 through 61 are handled specially
//const uint primesHandledWithSpecialCode = 27;	// Count of primes handled with inline code (not using primes array)
							// Primes 11 through 127 are handled specially
const uint primesHandledWithSpecialCode = 50;		// Count of primes handled with inline code (not using primes array)
							// Primes 11 through 251 are handled specially
//const uint primesHandledWithSpecialCode = 93;	// Count of primes handled with inline code (not using primes array)
							// Primes 11 through 509 are handled specially
#endif

// Various useful constants

const uint primesBelow64K = 6542;			// There are 6542 16-bit primes
const uint primesBelow128K = 12251;			// There are 12251 17-bit primes
const uint primesBelow1M = 82025;			// There are 82025 20-bit primes
const uint sieving64KCrossover = (primesBelow64K - primesNotSieved - primesHandledWithSpecialCode) / threadsPerBlock;
							// Number of thread loops processing primes below 64K
const uint sieving128KCrossover = (primesBelow128K - primesNotSieved - primesHandledWithSpecialCode) / threadsPerBlock;
							// Number of thread loops processing primes below 128K
const uint sieving1MCrossover = (primesBelow1M - primesNotSieved - primesHandledWithSpecialCode) / threadsPerBlock - 3;  // bug - awkward hard coded -3 here
...
uint primes_per_thread = 0;		// Number of "rows" in the GPU sieving info array that each thread processes
...
#define bit_to_clr	pinfo16
	...
	for (j = 0; ; ) {
		mask = (ulong) 1 << i67;
		mask |= ((ulong) 1 << i71) | ((ulong) 1 << i73);
		mask |= ((ulong) 1 << i79) | ((ulong) 1 << i83);
		mask |= ((ulong) 1 << i89) | ((ulong) 1 << i97);
	...
	for (j = 0; ; ) {
		mask = (ulong) 1 << i101;
		mask |= ((ulong) 1 << i103) | ((ulong) 1 << i107);
		mask |= ((ulong) 1 << i109) | ((ulong) 1 << i113);
		mask |= (ulong) 1 << i127;
	...
	for (j = 0; ; ) {
		mask1 = ((ulong) 1 << i131) | ((ulong) 1 << i137);
		mask1 |= ((ulong) 1 << i139) | ((ulong) 1 << i149);
		mask1 |= ((ulong) 1 << i151) | ((ulong) 1 << i157);
		mask2 = ((ulong) 1 << (i131 - 64)) | ((ulong) 1 << (i137 - 64));
		mask2 |= ((ulong) 1 << (i139 - 64)) | ((ulong) 1 << (i149 - 64));
		mask2 |= ((ulong) 1 << (i151 - 64)) | ((ulong) 1 << (i157 - 64));
	...
	for (j = 0; ; ) {
		mask1 = ((ulong) 1 << i163) | ((ulong) 1 << i167);
		mask1 |= ((ulong) 1 << i173) | ((ulong) 1 << i179);
		mask1 |= ((ulong) 1 << i181) | ((ulong) 1 << i191);
		mask2 = ((ulong) 1 << (i163 - 64)) | ((ulong) 1 << (i167 - 64));
		mask2 |= ((ulong) 1 << (i173 - 64)) | ((ulong) 1 << (i179 - 64));
		mask2 |= ((ulong) 1 << (i181 - 64)) | ((ulong) 1 << (i191 - 64));
	...
	for (j = 0; ; ) {
		mask1 = ((ulong) 1 << i193) | ((ulong) 1 << i197);
		mask1 |= ((ulong) 1 << i199) | ((ulong) 1 << i211);
		mask1 |= ((ulong) 1 << i223) | ((ulong) 1 << i227);
		mask2 = ((ulong) 1 << (i193 - 64)) | ((ulong) 1 << (i197 - 64));
		mask2 |= ((ulong) 1 << (i199 - 64)) | ((ulong) 1 << (i211 - 64));
		mask2 |= ((ulong) 1 << (i223 - 64)) | ((ulong) 1 << (i227 - 64));
	...
	for (j = 0; ; ) {
		mask1 = (ulong) 1 << i229;
		mask1 |= ((ulong) 1 << i233) | ((ulong) 1 << i239);
		mask1 |= ((ulong) 1 << i241) | ((cl_ulong) 1 << i251);
		mask2 = (ulong) 1 << (i229 - 64);
		mask2 |= ((ulong) 1 << (i233 - 64)) | ((ulong) 1 << (i239 - 64));
		mask2 |= ((ulong) 1 << (i241 - 64)) | ((ulong) 1 << (i251 - 64));
