...
#ifdef GWDEBUG
	if ((cl_uint) pinv != gen_sloppy_pinv (p))
		printf ("p doesn't match pinv!! p = %d, pinv = %d\n", p, pinv);
	if (r < -p / 2 || r >= p)
...
#define locsieve32	((cl_uint *) locsieve)
#define locsieve32v	((volatile cl_uint *) locsieve)
...
#define locsieve32	((cl_uint *) locsieve)
#define locsieve64	((cl_ulong *) locsieve)
#define pinfo16		((cl_ushort *) pinfo_dev)
#define pinfo32		((cl_uint *) pinfo_dev)
...
#define bit_to_clr	pinfo16
	...
	{
	cl_uint mask, mask2, mask3, mask4, i11, i13, i17, i19, i23, i29, i31, i37, i41, i43, i47, i53, i59, i61;
	...
	cl_uint i67, i71, i73, i79, i83, i89, i97, i101, i103, i107, i109, i113, i127;
	cl_ulong	mask;
	...
	for (j = 0; ; ) {
		mask = (cl_ulong) 1 << i67;
		mask |= ((cl_ulong) 1 << i71) | ((cl_ulong) 1 << i73);
	...
	for (j = 0; ; ) {
		mask = (cl_ulong) 1 << i101;
		mask |= ((cl_ulong) 1 << i103) | ((cl_ulong) 1 << i107);
	...
	cl_uint 	i131, i137, i139, i149, i151, i157, i163, i167, i173, i179, i181, i191;
	cl_uint 	i193, i197, i199, i211, i223, i227, i229, i233, i239, i241, i251;
	cl_ulong	mask1, mask2;
	...
	for (j = 0; ; ) {
		mask1 = ((cl_ulong) 1 << i131) | ((cl_ulong) 1 << i137);
		mask1 |= ((cl_ulong) 1 << i139) | ((cl_ulong) 1 << i149);
		mask1 |= ((cl_ulong) 1 << i151) | ((cl_ulong) 1 << i157);
		mask2 = ((cl_ulong) 1 << (i131 - 64)) | ((cl_ulong) 1 << (i137 - 64));
	...
	for (j = 0; ; ) {
		mask1 = ((cl_ulong) 1 << i163) | ((cl_ulong) 1 << i167);
		mask1 |= ((cl_ulong) 1 << i173) | ((cl_ulong) 1 << i179);
		mask1 |= ((cl_ulong) 1 << i181) | ((cl_ulong) 1 << i191);
		mask2 = ((cl_ulong) 1 << (i163 - 64)) | ((cl_ulong) 1 << (i167 - 64));
	...
	for (j = 0; ; ) {
		mask1 = ((cl_ulong) 1 << i193) | ((cl_ulong) 1 << i197);
		mask1 |= ((cl_ulong) 1 << i199) | ((cl_ulong) 1 << i211);
		mask1 |= ((cl_ulong) 1 << i223) | ((cl_ulong) 1 << i227);
		mask2 = ((cl_ulong) 1 << (i193 - 64)) | ((cl_ulong) 1 << (i197 - 64));
	...
	for (j = 0; ; ) {
		mask1 = (cl_ulong) 1 << i229;
		mask1 |= ((cl_ulong) 1 << i233) | ((cl_ulong) 1 << i239);
		mask1 |= ((cl_ulong) 1 << i241) | ((cl_ulong) 1 << i251);
		mask2 = (cl_ulong) 1 << (i229 - 64);
...
__kernel static void __attribute__((work_group_size_hint(256, 1, 1))) CalcModularInverses (cl_uint exponent, __global int *calc_info)
{
	...
	cl_ulong	facdist;	// Distance between two successive factors in a class
...
// The modular inverse is one over the distance between the corresponding factors for two successive k values in a class.
	...
	facdist = (cl_ulong) (2 * NUM_CLASSES) * (cl_ulong) exponent;
	calc_info[MAX_PRIMES_PER_THREAD*4 + index * 2 + 1] = modularinverse ((cl_uint) (facdist % prime), prime);
}
...
__kernel static void __attribute__((work_group_size_hint(256, 1, 1))) CalcBitToClear (cl_uint exponent, int96 k_base, __global int *calc_info, __global cl_uchar *pinfo_dev)
{
	...
	cl_uint	prime;		// Calculate the bit-to-clear of this prime number
	cl_uint	modinv;		// Distance between successive factors mod prime
	cl_uint	bit_to_clear;	// Calculated bit to clear
...
// Compute lowest possible value such that the factor (2 * k * exponent + 1) is divisible by our prime
	...
	cl_ulong	k_mod_p;	// k_base mod prime
	cl_ulong	factor_mod_p;	// factor mod prime

	k_mod_p = (((cl_ulong) k_base.d1 << 32) + k_base.d0) % prime;
	factor_mod_p = (2 * k_mod_p * exponent + 1) % prime;
	bit_to_clear = ((cl_ulong) prime - factor_mod_p) * modinv % prime;
